...
curve[i] = -0.001 * Math.exp(7 * x) ; // Expodec  carbon111.com/waveshaping1.html
//curve[i] = -0.001 * Math.exp(7 * -x); // Rexpodec carbon111.com/waveshaping1.html
...
var DCoffsetshaper = audioCtx.createWaveShaper(); // takes the data from DCoffset and applies it to the signal 
   ...
   // creating the other nodes
     ...
     var PulsaretVol = audioCtx.createGain();
...
// Making the connections between the nodes  
   ...
   arbitraryShaper.connect(PulsaretVol);
 
   PulsaretVol.connect(output);


//**********************************************************************************************************************      
// This buffer is used to play sound files for the pulsaret waveform
// Any sound file of any length can be used  


  var  pulsaretBuf;
  var  playit;
  
function pulsaretSound() { 
       
   var pulsaret = new XMLHttpRequest();              // start a XMLHttpRequest

pulsaret.open('GET', "PulsarertSoundFiles/ForkTineRing.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/S sound.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/ah sound.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/m sound.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/T Sound.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/not.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/shortCyclesOfNOT.wav", true);  
//pulsaret.open('GET', "PulsarertSoundFiles/Voice1.wav", true);
//pulsaret.open('GET', "PulsarertSoundFiles/Voice2.wav", true);
pulsaret.responseType = 'arraybuffer'; 
pulsaret.onload = function() {
audioCtx.decodeAudioData(pulsaret.response, function(buffer) {pulsaretBuf = buffer;});
 }
   pulsaret.send();
}  

function playPulsaret() {   
playit = audioCtx.createBufferSource();
playit.buffer = pulsaretBuf; 
playit.loop = true;
playit.playbackRate.value = 1.000;  
playit.connect(arbitraryShaper);                          
playit.connect(DCoffsetshaper);
playit.start(0);  

  console.log(playit);
}

//The pulsaretSound and playPulsaret functions are borrowed and reworked from (Smus 2011).
...
// "Trinagle", are borrowed from (Wolek 2014).  
...
//************************************************************************************************************************
//// These Three functions control the volume, play back rate and pitch of pulsaret buffer.


function changePulsaretVol() { // controls the volume of the pulsaret waveform
var volValue = parseFloat(document.getElementById("vol").value); // gets data from slider 
PulsaretVol.gain.value = volValue;
}    

function changePulsaretRate() { // controls the playback rate of the pulsaret buffer
var rateValue = parseFloat(document.getElementById("PulRate").value); // gets data from slider 
playit.playbackRate.value = rateValue;
}  

function changePulsaretPitch() { // controls the pitch of the pulsaret waveform
var pitchValue = parseFloat(document.getElementById("PulPitch").value); // gets data from slider 
playit.detune.value = pitchValue;  
} 
//************************************************************************************************************************
...
playit.disconnect(arbitraryShaper);     // disconnects the DCOffsetGain and osc nodes from arbitraryShaper
 
DCOffsetGain.connect(arbitraryShaper);

playit.connect(PulsaretVol);                 // reconnects the DCOffsetGain and osc nodes to the output gain node
DCOffsetGain.connect(PulsaretVol);
